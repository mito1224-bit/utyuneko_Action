...
public class DataManager : MonoBehaviour
{
    ...
    private void InitializeStage()
    {
        ...
        // ★追加：チェックポイントの記憶は無いが、このステージが「クリア済み」の場合
        //         （＝ステージ選択などから、既にクリアしたステージへ改めて挑戦しているケース）
        else if (GameManager.Instance != null && GameManager.Instance.IsStageCoreCubeCollected(currentSceneName))
        {
            // --- クリア済みステージの再挑戦：本番セーブデータのDataCube取得状況を引き継ぐ ---

            // ビットキューブ（通常コイン）は今回の周回分から数え直す
            // （ハイスコアはGameManager側のstage.maxBitCubesで別途管理されている）
            currentBitCubes = 0;

            // 本番セーブデータに記録済みのDataCube取得状況を復元する
            var savedFlags = GameManager.Instance.GetStageDataCubeFlags(currentSceneName);
            savedFlags.CopyTo(collectedStarCoinFlags);
            ...
            // チェックポイントは無いので、プレイヤーのスタート地点をそのまま記憶しておく
            GameObject player = GameObject.FindWithTag("Player");
            if (player != null) currentCheckpointPosition = player.transform.position;

            // ★ 既に取得済みのDataCubeオブジェクトをシーンから消去する（まっさらに戻さない）
            CleanUpCollectedStarCoins();

            Debug.Log($"【DataManager】クリア済みステージへの再挑戦：DataCube取得状況を復元しました（Data {currentDataCubes} / {maxDataCube}）");
        }
        else
        {
